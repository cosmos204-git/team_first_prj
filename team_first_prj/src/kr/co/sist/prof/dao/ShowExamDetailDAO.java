...
/*			
			select cs.course_code,cs.sub_code,exam_quest,exam_choice1,exam_choice2,exam_choice3,exam_choice4,exam_correct_tchoice
	 		 from exam_item ei, exam e , course_subject cs
			 where (cs.course_code=e.course_code and e.test_code =ei.test_code and cs.sub_code=e.sub_code)
			  and cs.course_code=(select course_code from course where course_name='FULL-STACK' )
			  and cs.sub_code=(select sub_code from subject where sub_name='HTML') ;
*/			
			
			// 과정과 과목기준으로 시험 문제, 보기1~4, 정답을 불러온다.
			strGetExamItemList
